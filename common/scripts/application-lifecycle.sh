...
do
	...
	if [ -e $WGTNAME-test.wgt ];then
		...
		if [ $? -eq 1 ];then
			if [ $SERVICE_USER -eq 1 -o $APPLICATION_USER -eq 1 ];then
				chown $AGLDRIVER /run/platform/debug/
			fi
	...
	if [ -e $WGTNAME-coverage.wgt ];then
		...
		if [ $? -eq 1 ];then
			...
			find /var/local/lib/afm/applications/ -iname htdocs -exec chsmack -a "*" {} \;
			find /var/local/lib/afm/applications/ -iname bin -exec chsmack -a "*" {} \;
			find /var/local/lib/afm/applications/ -iname etc -exec chsmack -a "*" {} \;
